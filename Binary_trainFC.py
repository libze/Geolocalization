...
for i, pth in enumerate(os.listdir(path)):
    ...
    if num_classes != 221:
        for i, label in enumerate(pth_data['labels']):
            if int_to_country[label.item()] == 'France':
                labels.append(0)
                features.append(pth_data['features'][i].numpy())
                #ids.append(pth_data['ids'][i].item())
            if int_to_country[label.item()] == 'United Kingdom':
                labels.append(1)
                features.append(pth_data['features'][i].numpy())
                #ids.append(pth_data['ids'][i].item())
            if int_to_country[label.item()] == 'Spain':
                labels.append(2)
                features.append(pth_data['features'][i].numpy())
                #ids.append(pth_data['ids'][i].item())
